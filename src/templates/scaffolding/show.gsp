		...
		<div id="show-${domainClass.propertyName}" class="content scaffold-show" role="main">
			...
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
				...
				<li class="fieldcontain">
					<span id="${p.name}-label" class="property-label"><g:message code="${domainClass.propertyName}.${p.name}.labelform" default="${p.naturalName}" /></span>
					<%  if (p.isEnum()) { %>
					...
					<%  } else if (p.type == Date || p.type == java.sql.Date || p.type == java.sql.Time || p.type == Calendar) { %>
						<span class="property-value" aria-labelledby="${p.name}-label"><amb:formatDate date="\${${propertyName}?.${p.name}}" /></span>
            ...
            <g:form>
				<fieldset class="buttons">
                    ...
                    <g:link class="delete" action="delete" id="\${${propertyName}?.id}" onclick="return confirm('Sei sicuro?');"><g:message code="default.button.delete.label" default="Delete" /></g:link>
				</fieldset>
			</g:form>
		</div>
	</body>
