...
//        testo += this.captionTabella(params)
    ...
    private static String corpoTabella(Croce croce, Date inizio, Date fine) {
        ...
        if (tipiTurno) {
            tipiTurno?.each {
                numTipoTurno++
                pariDispari++
                if (numTipoTurno in turniBordati) {
    ...
    private static String righeDiUnTurno(TipoTurno tipoTurno, Date inizio, Date fine, int pariDispari, int numExtra, boolean bordo) {
        ...
        if (funzioni) {
            ...
            funzioni?.each {
                testoRiga += this.rigaTurno(mappaTurniDiUnTipo, (Funzione) it, tipoTurno)
            } // fine del ciclo each
        }// fine del blocco if
...
//old            testoOut += Lib.tagController('Milite', 'Turni dei militi (statistiche)','statistiche')
    ...
    private static String rigaTurno(LinkedHashMap<Date, Turno> mappaTurni, Funzione funzione, TipoTurno tipoTurno) {
        ...
        if (mappaTurni) {
            mappaTurni.keySet().each {
                ...
                testoRiga += this.cellaTurno(giorno, turno, funzione, tipoTurno)
            }// end of each
        }// fine del blocco if
    ...
    private static String rigaOrario(int giorni, TipoTurno tipoTurno, int span, int pariDispari, boolean bordo) {
        ...
        testoRiga += this.cellaOrario(tipoTurno, bordo)

        //--celle dei giorni (7)
        for (int k = 0; k < giorni; k++) {
            if (bordo) {
                testoRiga += Lib.tagCella('', Aspetto.turnoorariobordo)
            } else {
     ...
     * Seconda colonna <br>
     */
    private static String cellaOrario(TipoTurno tipoTurno, boolean bordo) {
        String testoOut
        String testo = 'Manca'
        ...
        if (bordo) {
            testoOut = Lib.tagCella(testo, Aspetto.legendaorariobordo)
        } else {
...
//        testo += Lib.tagCella('Turno assegnato bloccato e non più modificabile')
    ...
    private static String copyright() {
        ...
        testo = 'Algos© 2013 - v1.4 del 18 marzo 2013'
